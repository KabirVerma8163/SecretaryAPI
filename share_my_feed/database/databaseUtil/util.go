...
func ClearAllCollections() error {
	_, err := LinksListsColl.DeleteMany(Ctx, bson.M{})
	if err != nil {
		return err
	}
	_, err = UsersColl.DeleteMany(Ctx, bson.M{})
	if err != nil {
		return err
	}
	_, err = UserDataColl.DeleteMany(Ctx, bson.M{})
	if err != nil {
		return err
	}
	_, err = DiscordConnectedAccountsColl.DeleteMany(Ctx, bson.M{})
	if err != nil {
		return err
	}
	return nil
...
func CheckUrl(link string) (string, error) {
	if !isURL(link) {
		return "", fmt.Errorf("InvalidURL: String: %v provided is not a valid URL", link)
	}
